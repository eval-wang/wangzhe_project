...
配合阶段3 的位置复盘，提供基于 20×20 网格地图的可视化标注工具（位于仓库根目录 `配套工具/`）：

| 工具 | 说明 |
|---|---|
| `配套工具/位置示意图.html` | 完整版标注器：固定背景图 + 围棋式落子（左键白棋 / 右键黑棋）+ CSV 导出/导入 |
| `配套工具/位置示意图-纯净版.html` | 纯净版：仅地图本体，页面与地图始终为正方形并自适应视口 |

- 交互：左键放置/移除白色棋子，右键放置/移除黑色棋子，再点同色棋子即移除；
- 数据：CSV 符号约定 `W`=白棋、`B`=黑棋，格式 `行,列,符号`，行、列从 0 开始；
- 背景：自动加载 `相关素材/地图/正方形.png`，也可手动更换。
